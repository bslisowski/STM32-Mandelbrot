...
static inline void _init_spi(uint16_t miso, uint16_t mosi, uint16_t sck) {
    ...
    SPI1->CR1 |= SPI_CR1_SSM_Msk | SPI_CR1_SSI_Msk | SPI_CR1_MSTR_Msk;
...
static inline void spi_write_hword(uint16_t byte) {
    while ((SPI1->SR & SPI_SR_TXE_Msk) == 0) spin(1);
    SPI1->DR = byte;
}

static inline void spi_write_buf(uint16_t *buf, size_t len) {
    while (len-- > 0) spi_write_hword(*buf++);
    while ((SPI1->SR & SPI_SR_TXE_Msk) == 0) spin(1);
